...
public class InputService : IInputService, IDisposable
{
    ...
    public void ApplySettings(KeyBindSettings settings)
    {
        _logger.LogInformation("Applying settings to input service");
        
        Stop();
        Start();
        
        foreach (var keyBinding in settings.Hotkeys)
        {
            RegisterHotkey(keyBinding);
        }
        
        
        _logger.LogInformation("Settings applied to input service");
    }
    ...
    private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
    {
        ...
        if (binding == null) return IntPtr.Zero;
        
        var mainWindow = Application.Current?.MainWindow?.GetType().Name;
        var isTargetWindow = mainWindow is "ChatWindow" or "MainWindow";
        if (!binding.IsEnabled || (!binding.IsGlobal && !isTargetWindow))
        {
            return IntPtr.Zero;
        }
